...
# ╔═╡ f06d2b00-1807-11f1-b79d-b1e68dc3769b
begin
    using DifferentialEquations
    using Plots
end

# ╔═╡ bf46f5d8-3793-4a1f-8e4c-021a15ca45ba
begin
    # Parameters
    g = 9.81
    L = 0.15

    # FAST rotation speed (change if your instructor gives a specific "fast" value)
    Ω_fast = 10.0   # rad/s
    ...
    # Equation of motion from Part 1 (simplified):
    # θ̈ = -(g/L) sin(θ) + 0.5 Ω^2 sin(2θ)
    function pendulum_fast!(du, u, p, t)
        θ, θdot = u
        du[1] = θdot
        du[2] = -(g/L) * sin(θ) + 0.5 * (Ω_fast^2) * sin(2*θ)
    end

    prob_fast = ODEProblem(pendulum_fast!, u0, tspan)
    sol_fast = solve(prob_fast, Tsit5(); reltol=1e-9, abstol=1e-9)

    sol_fast
end

# ╔═╡ b9f08145-6111-4ef5-9923-d44b85e9ae46
(sol_fast.t[end], sol_fast[1,end], sol_fast[2,end])

# ╔═╡ 4514bdfa-3662-4c08-a11f-a75b8fa3d694
plot(sol_fast.t, sol_fast[1,:],
    xlabel="t (s)", ylabel="θ (rad)",
    title="Fast rotation: θ(t)", label=false)
