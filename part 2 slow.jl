...
# ╔═╡ b2b7d88f-1f4f-47ab-9eb2-a37f52153a2b
md"""
## Part 2: Solve for Slow and Fast Rotation Speeds

This cell solves the same equation of motion for two different frame speeds:

- slow rotation: `Ω = 2.0 rad/s`
- fast rotation: `Ω = 12.0 rad/s`
"""

# ╔═╡ 3d3f77a6-79d7-4ef9-8eb1-865fd31f8925
begin
	# Equation from Part 1:
	# θ¨ = -(g/L)sin(θ) + Ω^2 sin(θ)cos(θ)
	omega_slow = 2.0
	omega_fast = 12.0

	initial_state_part2 = [0.15, 0.0]
	time_span_part2 = (0.0, 12.0)

	problem_slow = ODEProblem(
		pendulum_rotating!,
		initial_state_part2,
		time_span_part2,
		(length_L_val, gravity_g_val, omega_slow)
	)
	problem_fast = ODEProblem(
		pendulum_rotating!,
		initial_state_part2,
		time_span_part2,
		(length_L_val, gravity_g_val, omega_fast)
	)

	solution_slow = solve(problem_slow, Tsit5(); reltol = 1e-8, abstol = 1e-8)
	solution_fast = solve(problem_fast, Tsit5(); reltol = 1e-8, abstol = 1e-8)

	critical_omega = sqrt(gravity_g_val / length_L_val)

	p_angle = plot(
		solution_slow.t,
		getindex.(solution_slow.u, 1),
		label = "slow Ω = $(omega_slow) rad/s",
		xlabel = "time (s)",
		ylabel = "θ(t) (rad)",
		lw = 2,
		title = "Part 2: θ(t) comparison, Ωc = $(round(critical_omega, digits = 3)) rad/s"
	)
	plot!(
		p_angle,
		solution_fast.t,
		getindex.(solution_fast.u, 1),
		label = "fast Ω = $(omega_fast) rad/s",
		lw = 2,
		ls = :dash
	)
	...
	p_phase = plot(
		getindex.(solution_slow.u, 1),
		getindex.(solution_slow.u, 2),
		label = "slow Ω = $(omega_slow)",
		xlabel = "θ (rad)",
		ylabel = "θ̇ (rad/s)",
		lw = 2,
	...
	plot!(
		p_phase,
		getindex.(solution_fast.u, 1),
		getindex.(solution_fast.u, 2),
		label = "fast Ω = $(omega_fast)",
		lw = 2,
		ls = :dash
	)

	plot(p_angle, p_phase, layout = (2, 1), size = (820, 820))
end
