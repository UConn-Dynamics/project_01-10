...
# ╔═╡ 313630c0-177a-11f1-8c83-efb05e0b5307
begin
    using LinearAlgebra
    using Symbolics
    ...
    dL_dθdot = Symbolics.derivative(ℒ, D(θ))
    dL_dθ    = Symbolics.derivative(ℒ, θ)

	eq = expand_derivatives(D(dL_dθdot) - dL_dθ)
	
    θdd = Symbolics.solve_for(eq ~ 0, D(D(θ)))

    θdd_s1 = Symbolics.simplify(θdd)
    θdd_s2 = Symbolics.simplify_fractions(θdd_s1)
    θdd_s3 = Symbolics.expand(θdd_s2)
    θdd_simplified = Symbolics.simplify(θdd_s3)

    (
        θdd_raw = θdd,
        θdd_simplified = θdd_simplified
    )
end
...
manifest_format = "2.0"
project_hash = "13ab70ba9dc1e9ea6239b644136841f418db3160"
