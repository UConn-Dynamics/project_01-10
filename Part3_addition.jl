...
# ╔═╡ 13270450-18d9-11f1-bc90-a125b738c442
begin
	using DifferentialEquations
	using Plots
	using PlutoUI

	# Given geometry/params
	h1 = 0.2
	w1 = 0.1
	L  = 0.15
	g  = 9.81
	...
	# Tailored rotation speeds
	Ω_slow = 2.0
	Ω_fast = 12.0
	Ωc = sqrt(g / L)

	# Controls (optional)
	@bind tf Slider(5.0:1.0:30.0, default=12.0, show_value=true)
	@bind θ0 Slider(-pi/2:0.01:pi/2, default=0.15, show_value=true)
	@bind θdot0 Slider(-10.0:0.1:10.0, default=0.0, show_value=true)
	@bind do_gif CheckBox(default=false)

	# ODE: u = [θ, θ̇]
	function eom_rot!(du, u, p, t)
		θ, θdot = u
		Ω, L, g = p
		du[1] = θdot
		du[2] = -(g/L) * sin(θ) + (Ω^2) * sin(θ) * cos(θ)  # = -(g/L)sinθ + 0.5Ω^2 sin(2θ)
		return nothing
	end

	u0 = [θ0, θdot0]
	tspan = (0.0, tf)
	save_times = 0.0:0.01:tf

	sol_slow = solve(ODEProblem(eom_rot!, u0, tspan, (Ω_slow, L, g)), Tsit5();
		reltol=1e-9, abstol=1e-9, saveat=save_times
	)
	sol_fast = solve(ODEProblem(eom_rot!, u0, tspan, (Ω_fast, L, g)), Tsit5();
		reltol=1e-9, abstol=1e-9, saveat=save_times
	)
	...
	- Ωslow = **$(Ω_slow)** rad/s
	- Ωfast = **$(Ω_fast)** rad/s
	- Ωc = **$(round(Ωc, digits=3))** rad/s
	"""
end

# ╔═╡ 323ab74e-9ba7-428a-8097-7ea77bcd87e0
begin
	ts = sol_slow.t
	θ_s = sol_slow[1, :]
	θd_s = sol_slow[2, :]
	...
	p1 = plot(ts, θ_s; label="slow Ω=2.0",
		xlabel="t (s)", ylabel="θ (rad)",
		title="θ(t)  (Ωc=$(round(Ωc, digits=3)) rad/s)", lw=2
	)
	plot!(p1, ts, θ_f; label="fast Ω=12.0", lw=2, ls=:dash)

	p2 = plot(ts, θd_s; label="slow", xlabel="t (s)", ylabel="θ̇ (rad/s)", title="θ̇(t)", lw=2)
	plot!(p2, ts, θd_f; label="fast", lw=2, ls=:dash)

	p3 = plot(θ_s, θd_s; label="slow", xlabel="θ (rad)", ylabel="θ̇ (rad/s)", title="Phase portrait", lw=2)
	plot!(p3, θ_f, θd_f; label="fast", lw=2, ls=:dash)

	plot(p1, p2, p3; layout=(3,1), size=(860, 950))
end

# ╔═╡ ca8a940f-2318-4968-bcb5-bba17f24e53e
begin
	# Inertial (lab) coordinates including offsets w1,h1
	function xyz(θ, t; Ω, L, w1, h1)
		φ = Ω*t
		x = cos(φ)*L*sin(θ) - sin(φ)*w1
		y = sin(φ)*L*sin(θ) + cos(φ)*w1
		z = h1 - L*cos(θ)
		return x, y, z
	end

	# Analytic velocity in lab frame
	function vxyz(θ, θdot, t; Ω, L, w1, h1)
		φ = Ω*t
		sφ, cφ = sin(φ), cos(φ)

		xdot = -Ω*sφ*L*sin(θ) + cφ*L*cos(θ)*θdot - Ω*cφ*w1
		ydot =  Ω*cφ*L*sin(θ) + sφ*L*cos(θ)*θdot - Ω*sφ*w1
		zdot =  L*sin(θ)*θdot

		return xdot, ydot, zdot
	end

	function energies(sol; Ω, L, w1, h1, m, g)
		ts = sol.t
		T = similar(ts)
		V = similar(ts)
		E = similar(ts)
		...
		for i in eachindex(ts)
			...
			xd, yd, zd = vxyz(θ, θdot, t; Ω=Ω, L=L, w1=w1, h1=h1)
			v2 = xd^2 + yd^2 + zd^2

			T[i] = 0.5*m*v2
			_, _, z = xyz(θ, t; Ω=Ω, L=L, w1=w1, h1=h1)
			V[i] = m*g*z
			E[i] = T[i] + V[i]
		end
	...
	# NOTE: renamed time arrays to avoid Pluto "multiple definitions"
	tE_s, T_s, V_s, E_s = energies(sol_slow; Ω=Ω_slow, L=L, w1=w1, h1=h1, m=m, g=g)
	tE_f, T_f, V_f, E_f = energies(sol_fast; Ω=Ω_fast, L=L, w1=w1, h1=h1, m=m, g=g)

	pE1 = plot(tE_s, E_s; label="E slow", xlabel="t (s)", ylabel="Energy (J)", title="Total energy", lw=2)
	plot!(pE1, tE_f, E_f; label="E fast", lw=2, ls=:dash)

	pE2 = plot(tE_s, T_s; label="T slow", xlabel="t (s)", ylabel="J", title="Kinetic energy", lw=2)
	plot!(pE2, tE_f, T_f; label="T fast", lw=2, ls=:dash)

	pE3 = plot(tE_s, V_s; label="V slow", xlabel="t (s)", ylabel="J", title="Potential energy", lw=2)
	plot!(pE3, tE_f, V_f; label="V fast", lw=2, ls=:dash)

	plot(pE1, pE2, pE3; layout=(3,1), size=(860, 950))
end

# ╔═╡ 659b66a9-ca3c-414b-aa21-9cda6f203277
begin
	θgrid = range(-pi, pi; length=1400)

	# Effective potential for this reduced θ-dynamics:
	# V_eff(θ) = m g L (1 - cosθ) - 1/2 m Ω^2 L^2 sin^2θ
	Veff(θ; Ω) = m*g*L*(1 - cos(θ)) - 0.5*m*(Ω^2)*(L^2)*(sin(θ)^2)

	Vs = [Veff(θ; Ω=Ω_slow) for θ in θgrid]
	Vf = [Veff(θ; Ω=Ω_fast) for θ in θgrid]

	pV = plot(θgrid, Vs; label="slow Ω=2.0",
		xlabel="θ (rad)", ylabel="V_eff (J)",
		title="Effective potential V_eff(θ)", lw=2
	)
	plot!(pV, θgrid, Vf; label="fast Ω=12.0", lw=2, ls=:dash)

	# equilibrium points satisfy: sinθ(g - Ω^2 L cosθ)=0
	function eq_angles(Ω; L, g)
		angles = Float64[0.0, pi]
		if Ω > sqrt(g/L)
			c = g/(Ω^2*L)
			if abs(c) ≤ 1
				θeq = acos(c)
	...
	for θeq in eq_angles(Ω_slow; L=L, g=g)
		scatter!(pV, [θeq], [Veff(θeq; Ω=Ω_slow)]; label=false)
	end
	for θeq in eq_angles(Ω_fast; L=L, g=g)
		scatter!(pV, [θeq], [Veff(θeq; Ω=Ω_fast)]; label=false)
	end

	pV
end

# ╔═╡ 5a26e847-58ce-4211-af10-09d5e5aac3f6
begin
	Ωvals = range(0.0, 16.0; length=700)
	θeq_branch = fill(NaN, length(Ωvals))
	...
	pB = plot(Ωvals, zeros(length(Ωvals));
		label="θ=0 equilibrium",
		xlabel="Ω (rad/s)", ylabel="equilibrium θ (rad)",
		title="Equilibrium angle vs Ω (Ωc=$(round(Ωc, digits=3)) rad/s)", lw=2
	)
	plot!(pB, Ωvals,  θeq_branch; label="+θ*", lw=2)
	plot!(pB, Ωvals, -θeq_branch; label="-θ*", lw=2)
	vline!(pB, [Ωc]; label="Ωc", ls=:dash)

	# Mark your two chosen speeds
	scatter!(pB, [Ω_slow], [0.0]; label="Ωslow=2.0")
	if Ω_fast > Ωc
		cfast = g/(Ω_fast^2*L)
		θstar_fast = acos(cfast)
		scatter!(pB, [Ω_fast, Ω_fast], [θstar_fast, -θstar_fast]; label="Ωfast=12.0")
	else
		scatter!(pB, [Ω_fast], [0.0]; label="Ωfast=12.0")
	end

	pB
end

# ╔═╡ 205dd63f-b4a2-43f9-b4ae-14c7e7f5aeaa
begin
	function traj(sol; Ω, L, w1, h1)
		ts = sol.t
		x = similar(ts); y = similar(ts); z = similar(ts)
		for (i,t) in enumerate(ts)
			θ = sol[1,i]
			x[i], y[i], z[i] = xyz(θ, t; Ω=Ω, L=L, w1=w1, h1=h1)
		end
		return ts, x, y, z
	end

	ts_s, xs_s, ys_s, zs_s = traj(sol_slow; Ω=Ω_slow, L=L, w1=w1, h1=h1)
	ts_f, xs_f, ys_f, zs_f = traj(sol_fast; Ω=Ω_fast, L=L, w1=w1, h1=h1)

	p3d = plot3d(xs_s, ys_s, zs_s; label="slow Ω=2.0",
		xlabel="x (m)", ylabel="y (m)", zlabel="z (m)",
		title="3D bob trajectory (includes w1,h1)", lw=2
	)
	plot3d!(p3d, xs_f, ys_f, zs_f; label="fast Ω=12.0", lw=2, ls=:dash)

	pxy = plot(xs_s, ys_s; label="slow", xlabel="x (m)", ylabel="y (m)", title="Top view (x–y)", lw=2)
	plot!(pxy, xs_f, ys_f; label="fast", lw=2, ls=:dash)

	pxz = plot(xs_s, zs_s; label="slow", xlabel="x (m)", ylabel="z (m)", title="Side view (x–z)", lw=2)
	plot!(pxz, xs_f, zs_f; label="fast", lw=2, ls=:dash)

	plot(p3d, pxy, pxz; layout=(3,1), size=(860, 1100))
end

# ╔═╡ e470354d-929b-4b20-9e87-93ead70cc425
begin
	if do_gif
		# reuse xs_s,ys_s,zs_s etc from Cell 6
		stride = 10
		...
		anim = @animate for k in 1:stride:length(ts_s)
			plt = plot3d(; xlabel="x (m)", ylabel="y (m)", zlabel="z (m)",
				xlims=(minimum(xall), maximum(xall)),
				ylims=(minimum(yall), maximum(yall)),
				zlims=(minimum(zall), maximum(zall)),
				title="Rotating pendulum: slow solid, fast dashed   t=$(round(ts_s[k],digits=2)) s",
				legend=false
			)

			plot3d!(plt, xs_s[1:k], ys_s[1:k], zs_s[1:k]; lw=3)
			plot3d!(plt, xs_f[1:k], ys_f[1:k], zs_f[1:k]; lw=3, ls=:dash)

			scatter!(plt, [xs_s[k]], [ys_s[k]], [zs_s[k]]; markersize=5)
			scatter!(plt, [xs_f[k]], [ys_f[k]], [zs_f[k]]; markersize=5)
		...
		md"GIF is off — toggle the checkbox in Cell 1 to generate it."
	end
end
...
manifest_format = "2.0"
project_hash = "16f3c51cb9a2d1024becbb2b51ed09e26621059c"
...
git-tree-sha1 = "6d8aef5987fb40847eb52f33fee3cf6d4d0331df"
